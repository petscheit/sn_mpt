...
pub(crate) async fn list_batches(manager: Arc<ConnectionManager>) -> Result<impl warp::Reply, warp::Rejection> {
...
pub async fn fetch_batch(batch_id: u64, manager: Arc<ConnectionManager>) -> Result<impl Reply, warp::Rejection> {
    let conn = manager.get_connection().unwrap();
    let batch = db::batch::get_batch(&conn, batch_id).unwrap();
    Ok(warp::reply::json(&batch))
}

pub async fn create_batch(hex_values: Vec<String>, manager: Arc<ConnectionManager>) -> Result<impl warp::Reply, warp::Rejection> {
    // Convert each hex string to Vec<u8>
    let items: Vec<CachedItem> = hex_values.into_iter().map(|hex| {
        hex::decode(hex).map(|bytes| CachedItem::new(bytes)).unwrap()
    }).collect();
...
pub async fn update_batch_status(batch_id: u64, new_status: BatchStatus, manager: Arc<ConnectionManager>) -> Result<impl Reply, warp::Rejection> {
    let conn = manager.get_connection().unwrap();
    TrieCache::update_batch_status(&conn, batch_id, new_status).unwrap();

    Ok(warp::reply::with_status("Batch status updated", StatusCode::OK))
}
//
